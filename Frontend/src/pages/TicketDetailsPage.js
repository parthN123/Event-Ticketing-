...
// import QrCodeIcon from '@mui/icons-material/QrCode'; // Removed QrCodeIcon import
...
const TicketDetailsPage = () => {
  ...
  useEffect(() => {
    console.log('useEffect triggered in TicketDetailsPage.');
    const fetchTicket = async () => {
      console.log('fetchTicket function called.');
      try {
        setLoading(true);
        setError(null);
        console.log('Loading and error states reset.');

        if (!isAuthenticated) {
          setError('Please log in to view ticket details');
          setLoading(false);
          console.log('Not authenticated, exiting fetchTicket.');
          return;
        }
        ...
        console.error('Error fetching ticket details inside catch:', err);
  ...
  const generateQRValue = () => {
    if (!ticket || !ticket._id) {
      console.log('Ticket or Ticket ID not available for QR code (inside generateQRValue).', { ticket });
      return ''; // Return empty string if ticket or _id is missing
    }
    const baseUrl = window.location.origin;
    const qrValue = `${baseUrl}/tickets/${ticket._id}`;
    console.log('QR Code Value generated:', qrValue);
    return qrValue;
  };
  ...
  console.log("Ticket object in render:", ticket);
